...
function GlowCard({ children, className = '', glowColor = 'cyan', cardIndex = 0, onClick }: { children: React.ReactNode; className?: string; glowColor?: GlowColor; cardIndex?: number; onClick?: () => void; }) {
  ...
  return (
    <div ref={ref} className={`gc-wrap rounded-xl relative ${className}`} style={{ background: '#000' }} onClick={onClick}>
...
// Must match Services.tsx — same solid color, no backdrop-filter, no seam
const SECTION_BG = 'rgb(10, 10, 10)';
...
export default function Testimonials() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(([entry]) => { if (entry.isIntersecting) { setHasRevealed(true); observer.disconnect(); } }, { threshold: 0.15 });
  ...
  useEffect(() => {
    ...
    const onMove = (e: PointerEvent) => { const r = section.getBoundingClientRect(); const x = ((e.clientX - r.left) / r.width) * 100; border.style.setProperty('--tbx', `${x.toFixed(2)}%`); border.style.setProperty('--tb-opacity', '1'); };
    const onLeave = () => border.style.setProperty('--tb-opacity', '0');
    section.addEventListener('pointermove', onMove);
    section.addEventListener('pointerleave', onLeave);
    return () => { section.removeEventListener('pointermove', onMove); section.removeEventListener('pointerleave', onLeave); };
  }, []);
  ...
  const css = `
    .t-section-reveal { opacity: 0; transform: translateY(32px) scale(0.98); transition: opacity 0.9s cubic-bezier(0.22, 1, 0.36, 1), transform 0.9s cubic-bezier(0.22, 1, 0.36, 1); }
    .t-section-reveal.is-visible { opacity: 1; transform: translateY(0) scale(1); }
    .t-title-reveal { opacity: 0; transform: translateX(-12px); transition: opacity 0.7s ease 0.2s, transform 0.7s ease 0.2s; }
    .is-visible .t-title-reveal { opacity: 1; transform: translateX(0); }
    .t-marquee-reveal { opacity: 0; transform: translateX(-24px); transition: opacity 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.45s, transform 0.7s cubic-bezier(0.22, 1, 0.36, 1) 0.45s; }
    .is-visible .t-marquee-reveal { opacity: 1; transform: translateX(0); }
    ...
    .is-visible .t-typewriter { animation: t-typing 1.2s steps(20, end) 0.3s forwards, t-blink 0.75s step-end 3; animation-delay: 0.3s, 0.3s; animation-fill-mode: forwards; }
    @keyframes t-marquee { 0% { transform: translateX(0); } 100% { transform: translateX(-50%); } }
    .t-animate-marquee { animation: t-marquee 18s linear infinite; animation-play-state: paused; }
    .t-marquee-running .t-animate-marquee { animation-play-state: running; }
  ...
  return (
    <section ref={sectionRef} className="relative py-8 md:py-12 overflow-hidden" style={{ background: SECTION_BG }}>
      ...
      <div className={`t-section-reveal relative z-10 ${hasRevealed ? 'is-visible' : ''}`}>
        <div className="max-w-7xl mx-auto px-6 mb-4">
          <div className="inline-block t-title-reveal">
            <h3 className="t-typewriter text-lg md:text-xl font-black text-white tracking-tight uppercase">What People Say</h3>
          </div>
          <div className="mt-0.5 h-1 w-10 bg-white/50 rounded-full" />
        </div>

        <div className={`relative t-pause-marquee mask-fade t-marquee-reveal ${hasRevealed ? 't-marquee-running' : ''}`}>
          <div className="flex overflow-hidden py-4">
            <div className="flex gap-4 t-animate-marquee whitespace-nowrap pl-6">
              {infiniteReviews.map((review, index) => (
                <GlowCard key={index} glowColor={glowPalette[index % glowPalette.length]} cardIndex={index} onClick={() => setSelectedReview(index % reviews.length)} className="flex-shrink-0 w-[245px] md:w-[300px] cursor-pointer group whitespace-normal">
                  <div className="flex flex-col h-full p-4">
                    <p className="text-white/95 text-[14px] leading-[20px] mb-3 font-medium italic flex-1">"{review.review}"</p>
                    <div className="flex items-center gap-2 mt-auto">
                      <div className="w-5 h-5 rounded-full bg-white/15 border border-white/25 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <a href={review.link} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="hover:opacity-60 transition-opacity">
                          <span className="text-white font-black text-[9px] block truncate uppercase leading-none underline decoration-white/20 decoration-1 underline-offset-2">{review.name}</span>
                          <span className="text-white/75 text-[6.5px] font-bold uppercase tracking-wider leading-none mt-0.5 block">{review.role}</span>
                        </a>
                      </div>
                      <div className="text-white/50 group-hover:text-white/80 transition-colors">{getIcon(review.icon)}</div>
                    </div>
                  </div>
                </GlowCard>
              ))}
      ...
      <div ref={bottomBorderRef} className="bottom-border-glow pointer-events-none absolute bottom-0 left-0 right-0 z-10" style={{ height: '1.2px' }}>
      ...
      {selectedReview !== null && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center p-4" onClick={() => setSelectedReview(null)}>
          <div className="relative max-w-sm w-full" onClick={(e) => e.stopPropagation()}>
            <GlowCard glowColor={glowPalette[selectedReview % glowPalette.length]} cardIndex={selectedReview} className="w-full">
              <div className="p-6">
                <button onClick={() => setSelectedReview(null)} className="absolute top-3 right-3 p-1.5 hover:bg-white/10 rounded-full transition-colors z-10"><X className="w-4 h-4 text-white/70" /></button>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-8 h-8 rounded-full bg-white/10 border border-white/25 flex-shrink-0" />
                  <div>
                    <a href={reviews[selectedReview].link} target="_blank" rel="noopener noreferrer" className="hover:opacity-70 transition-opacity">
                      <span className="font-black text-xs uppercase block text-white">{reviews[selectedReview].name}</span>
                      <span className="text-[9px] font-bold text-white/70 uppercase block">{reviews[selectedReview].role}</span>
                    </a>
                  </div>
                </div>
                <p className="text-base font-medium italic leading-snug text-white/90">"{reviews[selectedReview].review}"</p>
              </div>
            </GlowCard>
          </div>
        </div>
