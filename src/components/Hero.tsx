...
function AntigravityParticles() {
  ...
  useFrame((state) => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // 2. Magnetic Pull: Particles move toward normalized mouse coords
  ...
  return (
    <points ref={points}>
      ...
      <pointsMaterial
        ...
        blending={THREE.MultiplyBlending} // Ensures they look good on white
      />
    </points>
  );
}
  ...
  useEffect(() => {
    ...
    const type = () => {
      ...
      if (!isDeleting) {
        ...
        currentText = phrase.slice(0, currentText.length - 1);
        speed = 40;
        if (currentText === "") { isDeleting = false; setPhraseIdx((prev) => (prev + 1) % placeholderPhrases.length); speed = 1000; }
      }
    ...
    <section className="relative min-h-screen md:min-h-[170vh] flex flex-col items-center bg-white text-black overflow-x-hidden pt-36 pb-24" style={{ fontFamily: 'Georgia, serif' }}>
      ...
      <div className={`fixed inset-0 pointer-events-none z-0 transition-opacity duration-[2000ms] ${showParticles ? 'opacity-100' : 'opacity-0'}`}>
        <Canvas camera={{ position: [0, 0, 5], fov: 75 }}>
          ...
          <EffectComposer>
            <Bloom luminanceThreshold={0.5} intensity={0.4} radius={0.2} />
          </EffectComposer>
        </Canvas>
      </div>
