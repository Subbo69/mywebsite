...
export default function Hero({ onBookingClick, onAskAIClick, language }: HeroProps) {
  ...
  const handleAISubmit = (e: React.FormEvent) => {
    ...
    if (messageCount >= 25) { setErrorMessage("Message limit reached (25/25). Please contact us for more."); return; }
    if (query.length > 2000) { setErrorMessage("Character limit exceeded (Max 2000)."); return; }
    onAskAIClick(query);
  ...
  return (
    ...
    >
      <style>{`
        ...
        .animate-bounce-down { animation: bounce-down 1.1s ease-in-out infinite; }
      `}</style>
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 w-full max-w-7xl h-full">
        ...
        {/* Title: all chars always in DOM so layout never reflows.
            Untyped chars are transparent; cursor injected after last typed char. */}
        <div className="min-h-[120px] md:min-h-[180px] flex items-center justify-center w-full mb-6">
          <h1 className="text-4xl md:text-8xl font-bold tracking-tight" style={{ fontFamily: '"Montserrat", sans-serif' }}>
            {displayText.length === 0 && <span className="typewriter-cursor" />}
            {fullText.split('').map((char, i) => (
              <span key={i}>
                <span style={{ color: i < displayText.length ? 'inherit' : 'transparent' }}>
                  {char === ' ' ? '\u00A0' : char}
                </span>
                {i === displayText.length - 1 && displayText.length < fullText.length && (
        ...
        <div className="flex flex-col items-center gap-12 w-full max-w-md mt-4">
          ...
          <div className={`w-full space-y-3 transition-all duration-1000 ${showInput ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 pointer-events-none'}`}>
            {/* Label + bold black bouncing arrow */}
            <div className="flex flex-col items-center gap-2">
              ...
              {!isSent && (
                <ChevronDown className="w-6 h-6 text-black animate-bounce-down" strokeWidth={2.5} />
              )}
            </div>
