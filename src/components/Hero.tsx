...
export default function Hero({ onBookingClick, onAskAIClick, language }: HeroProps) {
  ...
  useEffect(() => {
    ...
    const type = () => {
      if (!mounted) return;
      if (i <= fullText.length) { setDisplayText(fullText.slice(0, i++)); setTimeout(type, 50); }
  ...
  // ─── Particle Engine (Simplified for performance) ────────────────────────
  useEffect(() => {
    ...
    const init = () => {
      ...
      for (let i = 0; i < cfg.pills; i++) {
        ...
        const ox = cx + Math.cos(angle) * dist; const oy = cy + Math.sin(angle) * dist; const col = COLOR_BANDS[Math.floor(Math.random() * COLOR_BANDS.length)];
        const size = 1.5 + Math.random() * 3.5;
        particles.push({ x: ox, y: oy, originX: ox, originY: oy, vx: 0, vy: 0, angle: Math.random() * Math.PI, width: size * 3, height: size * 0.6, r: col.r, g: col.g, b: col.b, opacity: 0.5 });
      }
    };
  ...
  const handleAISubmit = (e: React.FormEvent) => {
    ...
    if (query.length > 2000) { setErrorMessage("Limit: 2000 characters."); return; }
    onAskAIClick(query);
  ...
  return (
    <section className="relative min-h-screen flex flex-col items-center bg-white text-black overflow-x-hidden pt-12 md:pt-20 pb-12" style={{ fontFamily: 'Georgia, serif' }}>
      <style>{`
      ...
      <div className="relative z-10 flex flex-col items-center text-center px-6 w-full max-w-6xl">
        {/* Title: Scaled down to prevent overflow */}
        <div className="min-h-[120px] md:min-h-[220px] flex items-center justify-center w-full mb-4">
          <h1 className="text-4xl md:text-6xl lg:text-7xl font-bold tracking-tight" style={{ fontFamily: '"Montserrat", sans-serif' }}>
        ...
        {/* Subtitle */}
        <p className={`text-base md:text-xl text-zinc-500 mb-8 max-w-2xl font-light italic transition-all duration-[1000ms] ${showSubtitle ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
          {t.heroSubtitle}
        </p>

        {/* CTA & Input Group: Tightened spacing */}
        <div className="flex flex-col items-center gap-8 w-full max-w-lg">
          <button onClick={onBookingClick} className={`group bg-black text-white px-8 py-4 rounded-full text-base font-medium flex items-center gap-2 hover:scale-105 transition-all duration-700 shadow-xl ${showCTA ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8 pointer-events-none'}`}>
          ...
          <div className={`w-full space-y-3 transition-all duration-1000 ${showInput ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12 pointer-events-none'}`}>
            <div className="flex flex-col items-center">
              <h3 className="text-[11px] md:text-xs uppercase tracking-[0.4em] font-black text-black">
                {isSent ? t.openingChat : t.askAiAgent}
              </h3>
              {!isSent && <ChevronDown className="w-5 h-5 text-blue-500 animate-bounce-small" />}
            </div>

            <form onSubmit={handleAISubmit} className="relative flex items-center bg-white border-2 border-black rounded-2xl p-1.5 shadow-2xl transition-all focus-within:border-blue-500">
              <input
        ...
        {/* Video Preview: Positioned so it doesn't crowd the top content */}
        <div className="w-full max-w-5xl mt-24 md:mt-36 transition-all duration-700" style={{ opacity: scrollOpacity, transform: `scale(${scrollScale})` }}>
          <div ref={videoContainerRef} onClick={() => setIsModalOpen(true)} onMouseMove={handleMouseMove} onMouseEnter={() => setIsHoveringVideo(true)} onMouseLeave={() => setIsHoveringVideo(false)} className={`group relative aspect-video w-full rounded-[2rem] overflow-hidden shadow-2xl bg-black border border-zinc-100 ${isTouch ? 'cursor-pointer' : 'cursor-none'}`}>
            {!isTouch && (
